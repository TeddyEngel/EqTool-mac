...
namespace EQTool.Avalonia.Controls
{
    ...
    public class ZoneMapControl : Control
    {
        ...
        {
            hasFitToViewport = false;
            transformedBounds = ComputeTransformedBounds(MapData);
        }

        // EQ (x, y) -> screen (-y, -x).
        private static Point ToScreenSpace(double eqX, double eqY)
        {
            return new Point(-eqY, -eqX);
        }

        private static Rect ComputeTransformedBounds(ParsedData data)
        {
        ...
        {
            var screenPoint = ToScreenSpace(eqX, eqY);
            return new Point(
